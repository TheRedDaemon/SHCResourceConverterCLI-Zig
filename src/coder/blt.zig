...
pub fn blt(
    ...
    if (source.len != source_width * source_height or destination.len != destination_width * destination_height) {
...
test "blt" {
    ...
    @memset(source, 1);
    const target: []u8 = try std.testing.allocator.alloc(u8, destination_width * destination_height);
